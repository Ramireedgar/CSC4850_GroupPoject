...
result = {
    "model": "Neural Network (PCA XAI)",
    "test_accuracy": round(acc, 4)
}
with open(os.path.join(script_dir, 'nn_pca_xai_result.json'), 'w') as f:
...
plt.savefig(os.path.join(results_dir, 'shap_nn_pca_summary_plot.png'), bbox_inches='tight')
plt.close()
...
plt.savefig(os.path.join(results_dir, 'shap_nn_pca_local_match.png'), bbox_inches='tight')
plt.close()
